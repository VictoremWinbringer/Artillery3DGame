...
namespace DifferedRendering
{
    ...
    class Model : IDisposable
    {
        ...
        public Model(SharpDX.Direct3D11.Device device, string modelFile, string Folder, int Width, int Height)
        {
            _proj = Matrix.PerspectiveFovLH(MathUtil.PiOverFour,Width / (float)Height, 1f, 10000f);
            _bones = new BonesConst();
            _device = device;
            _model = new ModelSDX(_device, Folder, modelFile);
            _shader = new Shader(_device.ImmediateContext, "Shaders\\Assimp.hlsl", SkinnedPosNormalTexTanBi);

            _samler = new SamplerState(_device, new SamplerStateDescription()
            {
            ...
            _constBuffer0 = new SharpDX.Direct3D11.Buffer(_device, Utilities.SizeOf<AnimConst>(), ResourceUsage.Default, BindFlags.ConstantBuffer, CpuAccessFlags.None, ResourceOptionFlags.None, 0);
            _constBuffer1 = new SharpDX.Direct3D11.Buffer(_device, BonesConst.Size(), ResourceUsage.Default, BindFlags.ConstantBuffer, CpuAccessFlags.None, ResourceOptionFlags.None, 0);
        }
